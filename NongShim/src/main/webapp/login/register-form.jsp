...
<form action="${pageContext.request.contextPath}/RegisterController.do" method="post" onsubmit="register(event)" onsubmit="checkRegisterForm(event)">
   <table class = "table-bordered table-borderless">
      ...
      <tr>
         <td></td><td><input type = "text" name = "tel" id="memberTel" required="required" placeholder="전화번호" 
         onkeyup="checkTel()" style="height: 40px;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
         </td><td style="text-align: left;"><span id="checkResult2" >
         </span></td>
      </tr>
      <tr>
         <td></td><td><input type = "text" name = "accountNo" id="memberAccountNo" required="required" placeholder="계좌번호" onkeyup="checkAccountNo()" style="height: 40px;"
         	oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
         	</td><td style="text-align: left;"><span id="checkResult3"></span></td>
      </tr>
      <tr>
...
function checkRegisterForm2(event){
	if(checkFlag2==false){
		alert("번호 인증을 받으세요");
		event.preventDefault();
	}
	...
	function checkAccountNo(){
		...
		let memberAccountNo=document.getElementById("memberAccountNo").value; 
		let checkResultSpan=document.getElementById("checkResult3");
		
		if(memberAccountNo.length<13){
			checkResultSpan.innerHTML="<font color=pink>계좌번호는 13자 이상이여야 합니다.</font>";
		}else{
			let xhr=new XMLHttpRequest();
			xhr.onreadystatechange=function(){
				if(xhr.readyState==4&&xhr.status==200){
					if(xhr.responseText=="ok"){
						checkResultSpan.innerHTML="<font color=blue>사용가능한 계좌번호입니다.</font>";
						checkFlag=true;
					}else{
						checkResultSpan.innerHTML="<font color=red>계좌번호를 다시 입력하세요</font>";
					}
				}//if
			}//function
			xhr.open("get","${pageContext.request.contextPath}/CheckAccountNoController.do?accountNo="+memberAccountNo);
			xhr.send();	
		}
	}
