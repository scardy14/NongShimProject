...
<img src="images/share.png" onclick="urlClip()"
	style="width: 40px; height: 40px;">
&nbsp;
<img src="images/like.png"
	style="width: 50px; height: 50px; background-color: background;"
	onclick="startAjax12()">
...
<table class="">
	<tr>
		<td rowspan="2" style="width: 1000px;"><textarea
				style="width: 100%; height: 80px;" id="comment" required="required"
				placeholder="댓글입력"></textarea></td>
		<td>
			<button type="button"
				style="background-color: #00ba00; border-radius: 10px; height: 40px; border: none;"
				onclick="moon()">
				<span style="color: white;">문의글작성</span>
			</button>
		</td>
	</tr>
	<tr>
		<td>
			<button type="button"
				style="background-color: #00ba00; border-radius: 10px; height: 40px; border: none;"
				onclick="hoo()">
				<span style="color: white;">후기글작성</span>
			</button>
		</td>
	</tr>
</table>
...
<table class="table   boardlist">
	...
	<tbody>
		<c:forEach items="${commentlist}" var="commentlist"
			...
			<tr>
				<td><input type="hidden" id="comment_No${commentno.count }"
					value=${commentlist.comment_No }>${commentno.count}</td>
				...
				<c:choose>
					<c:when test="${commentlist.id==sessionScope.mvo.id }">
						...
						<td><button style="border: none; background-color: #00ba00;"
								onclick="updatecomment(${commentno.count})">
								<span style="color: white;">수정</span>
							</button></td>
						<td><button style="border: none; background-color: #00ba00;"
								onclick="deletecomment('${commentno.count}');">
