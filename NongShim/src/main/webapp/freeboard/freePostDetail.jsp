...
<div style="text-align: right;">
	<button type="button" onclick="toList()"
		style="background-color: #d0fd7d; border-radius: 10px; width: 80px; height: 40px; border: none;">목록으로</button>
	<c:choose>

		<c:when test="${sessionScope.mvo.id== FreePostVO.id}">
			<button type="button" onclick="updatepost()"
				style="background-color: #d0fd7d; border-radius: 10px; width: 80px; height: 40px; border: none;">수정하기</button>
			<button type="button" onclick="deletepost()"
				style="background-color: #d0fd7d; border-radius: 10px; width: 80px; height: 40px; border: none;">삭제하기</button>
		</c:when>
	</c:choose>
</div>
<br>
